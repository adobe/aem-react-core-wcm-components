...
export const ListV2ItemModificationDate = (item: ListV2Item) : JSX.Element => {
...
export const ListV2ItemContent = (item: ListV2Item) : JSX.Element => {
    return (
      <>
...
export const ListV2Anchor = (item: ListV2Item) : JSX.Element => {
...
export const ListV2ItemDescription = (item: ListV2Item) : JSX.Element => {
    return (
      <span className={item.baseCssClass + '__item-description'}>${item.description}</span>
    );
};

export const ListV2Impl = (props: ListV2Model) : JSX.Element => {

    const ListV2Item = (item: ListV2Item) : JSX.Element => {
    ...
    return (
      <ul className={props.baseCssClass}>
        {props.items.map((item, index) => <ListV2Item {...item} key={'cmp-list-' + index} baseCssClass={props.baseCssClass}
                                                          routed={item.routed} index={index}/>)}
      </ul>
    );
};
