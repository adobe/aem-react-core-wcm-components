...
export const BreadCrumbV2SSpan = (crumbItem: BreadCrumbV2ItemModel): JSX.Element => {
    return (
      <span itemProp="name">{crumbItem.title}</span>
    );
};

export const BreadCrumbV2Link = (crumbItem: BreadCrumbV2ItemModel): JSX.Element => {
    return (
      <RoutedLink
            to={crumbItem.url}
...
export const BreadCrumbV2ListItem = (crumbItem: BreadCrumbV2ItemModel): JSX.Element => {
      ...
      <nav className={props.baseCssClass}
        ...
        <ol className={props.baseCssClass + '__list'}
          ...
          {props.items.map((item, index) =>
            <BreadCrumbV2ListItem {...item} index={index} baseCssClass={props.baseCssClass} key={'crumbitem-' + index}/>)}
        </ol>
      </nav>
    );
...
const BreadCrumbV2 = (props: BreadCrumbV2Model): JSX.Element => {
